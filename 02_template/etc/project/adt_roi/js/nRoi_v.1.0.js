...
function nRoiSvg(option){
    ...
    var commonFunc = {
        ...
        deleteObj : function() {
            ...
            commonFunc.typeCountMinus(defaultArr[selectNum].areaType);
    ...
    function areaChange(tgObj){
        Object.defineProperty(tgObj, 'changeArea', {
            set: function(val) {
                this.areaType = val;
                selectObj.classList.remove('focus');
                selectObj.classList.remove('except');
                selectObj.classList.remove('ignore');
                selectObj.classList.add(this.areaType);
            }
        });
    }
    ...
    var methodArrow = {
        ...
        arrowDirSet : function(tgObj, dir){
            var rotateObj = tgObj.querySelector('g.arrow-angle'),
                nowDir    = rotateObj.getAttribute('data-dir'),
                rotateVal;
            if(nowDir == dir) return;
            rotateVal = commonFunc.getNum(rotateObj.getAttribute('transform')) + 180;
        ...
        addArrow : function(tg, array, dir){
            ...
            var newG1 = document.createElementNS("http://www.w3.org/2000/svg", 'g'),
                newG2 = document.createElementNS("http://www.w3.org/2000/svg", 'g'),
                newPl = document.createElementNS("http://www.w3.org/2000/svg", 'polyline'),
            ...
            newPl.setAttribute('points', '0,20 20,0 40,20');
            newPl.setAttribute('transform', 'translate(-20 -40)');
            ...
            newL.setAttribute('y2','80');
            newL.setAttribute('transform','translate(-20 -40)');
    ...
    var nameTextAdd = function(tgObj, name, x, y){
        ...
        newR.setAttribute('width', '100');
        newR.setAttribute('height', '20');
        newR.setAttribute('y', '0');
        newT.setAttribute('x', '4');
        newT.setAttribute('y', '14');
    ...
    var drawGroup = function(){
        ...
        nowDrawG.classList.add('drawing');
    ...
    drawEnd = function (){
        ...
        nowDrawG.classList.remove('drawing');
    ...
    function dataDraw(){
        var dataDrawPoly = function(arr, type, name, dir){ // polygon or polyline 생성
            ...
            var newElement = document.createElementNS("http://www.w3.org/2000/svg", 'g');
            nowDrawG = newElement;
            nowDrawG.classList.add(obj.areaType);
            svg.appendChild(nowDrawG);
            nowDrawG.setAttribute('data-num', drawNum);
            commonFunc.typeCountPlus(obj.areaType);
    ...
    // 영역 타입 변경 함수
    this.modifyArea = function(area){
        if(selectNum == null) {
            alert('영역을 선택해주세요.');
            return;
        }
        defaultArr[selectNum].changeArea = area;
    }
    ...
    // 영역 타입 갯수 재 확인
    this.updateTypeNum = function(){
        countFocus  = svgArea.querySelectorAll('.focus').length;
    ...
    // 도형 삭제
    this.objectDelete = function(num){
        selectNum = num;
        selectObj = svg.querySelector('[data-num="'+selectNum+'"]');
    ...
    this.setLineDir = function(dir){
        defaultArr[selectNum].direction = dir;
        methodArrow.arrowDirSet(selectObj.querySelector('.arrow'), dir);
    }
    // 화살표 방향 반대 (그리고 있는 도형)
    this.setLineDirIng = function(dir){
        methodArrow.arrowDirSet(nowDrawG, dir);
        direction = dir;
    }

    // 그리기 실행
    this.drawStart = function(area, obj, name){
        if(nowDrawG != null) {

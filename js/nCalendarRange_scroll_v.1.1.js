 ...
 * 
 * inPage (boolean) : 페이지 내 삽입여부 (기본은 false)
 * inTarget (dom/string) : inPage 가 true 일 경우 달력 넣을 영역 dom / 선택자
 * calTitle (string) : 달력 제목
 * drawLimit (string) : 달력 그리기 제한 연월 (ex : '2024-02')
 * beforeLimit (number) : 오늘 이전 몇일까지 선택가능하게할지 number 값 (ex : 20 - 오늘 기준 20일 전까지만 활성화)
 * select_gap (number) : 시작일 - 종료일 gap 제한 (ex: 30 - 시작일과 종료일 차이가 30일 이하일 경우만 선택 가능)
 * click_gap (number) : 시작일 - 종료일 gap 제한 (ex: 30 - 시작일과 종료일 차이가 30일 이하일 경우만 선택 가능)
 * shot_year (boolean) : 연도 표기를 2자리로 할지 여부 (기본은 false)
 * enabled_arr (array) : 활성화할 버튼 배열 (각 배열의 값은 'YYYY-MM-DD' 형식)
...
function nCalendarRangeScroll(option){
    let inPage          = option.inPage ? option.inPage : false,
        ...
        inp_single_cls  = option.inp_single_cls != undefined ? option.inp_single_cls : '.cal',
        calTitle        = option.calTitle,
        drawLimit       = option.drawLimit ? option.drawLimit : '2000-01',
        beforeLimit     = option.beforeLimit ? option.beforeLimit : null,
        select_gap      = option.select_gap != undefined ? option.select_gap : null,
        click_gap       = option.click_gap != undefined ? option.click_gap : null,
        shot_year       = option.shot_year ? option.shot_year : false,
        enabled_arr     = option.enabled_arr,
    ...
    if(inPage == false) {
        ...
        if(inp_single == true) inp_start = wrap.querySelector(inp_single_cls);
        else {
    ...
    let now         = new Date(),
        ...
        yearArr      = new Array(), // 오늘 / 90일전 / 그 사이 날짜를 포함하는 월의 배열 */
        cal_height,
        dateBtns,
        limitDay,
        drawLimitDate = new Date(drawLimit.split('-')[0], drawLimit.split('-')[1] - 1, 1);
    ...
    function calShow(){
        ...
        if(btn_close.getAttribute('tabindex', '-1')) {
            btn_close.removeAttribute('tabindex');
            btn_close.removeAttribute('aria-hidden');
        }
        setTimeout(function(){ calScrollSet(); }, 100); // ios 보정용
    }
    ...
    /** 컨펌 버튼 활성화 제어 */
    function btn_dis_ctrl(state){
        if(startDate != null && endDate != null) {
            btn_confirm.disabled = false;
            btn_confirm.focus();
            btn_confirm.removeAttribute('tabindex');
            btn_confirm.removeAttribute('aria-hidden');
        } else {
            btn_confirm.disabled = true;
            btn_confirm.setAttribute('tabindex', -1);
            btn_confirm.setAttribute('aria-hidden', true);
        }
    }
    ...
    function monCalc(){
        ...
        yearArr = tempArr.filter((item, idx) => tempArr.indexOf(item) === idx);
    }

    /** yearArr 기준 이전달 산출 */
    function calcPrevMon(){
        let nowFirst = yearArr[yearArr.length - 1],
            splitTx = nowFirst.split('-'),
            tempY   = Number(splitTx[0]),
            tempM   = Number(splitTx[1]) - 1,
            tgDate  = new Date(tempY, tempM, 1);
        if(tgDate.getTime() < drawLimitDate.getTime()) { // 산출한 이전달이 그리기 제한월보다 적을 경우
            obsr1.disconnect();
            return false;
        }
    ...
    /**
     * yearArr에 따라 달력 그리기 실행
     * @param {array} arr - yearArr
     */
    ...
    function preMonAdd(ent){
        ent.forEach((entry) => {
            if(entry.isIntersecting) {
                let tgDate = calcPrevMon();
                if(tgDate == false) return;
                yearArr.push(tgDate);
                calDrawRun(tgDate);
    ...
    function dateStateSet(e){
		let tg          = e,
            ...
            thisValue   = convertToDate(tgDate, '-').getTime();
        btn_ena(tg);
        if(thisValue > today.getTime()) btn_dis(tg);
        if(beforeLimit != null && thisValue < limitDay.getTime()) btn_dis(tg);
        if(thisValue == today.getTime()) tg.parentNode.classList.add('today');

        if(startDate) if(thisValue == startDate.getTime()) tg.parentNode.classList.add('start', 'in-range');
        if(endDate) {
            if(thisValue == endDate.getTime()) tg.parentNode.classList.add('end', 'in-range');
            if(thisValue > startDate.getTime() && thisValue < endDate.getTime()) tg.parentNode.classList.add('in-range');
        };
        if(enabled_arr != undefined) {
            if(enabled_arr.indexOf(tgDate) > -1) return;
            btn_dis(tg); 
        } 
	}

    /** 시작일 클릭 시 - 선택가능버튼 외 비활성화 설정 */
    function click_gap_set(){
        let limitDate = new Date(startDate.getFullYear(), startDate.getMonth(), startDate.getDate() + click_gap);
        dateBtns.forEach((btn)=>{
            let tgDate      = btn.getAttribute('data-date'),
                thisValue   = convertToDate(tgDate, '-').getTime();
            if(thisValue < startDate.getTime() || thisValue > limitDate.getTime()) btn_dis(btn);
        });
    }
    ...
    function dateBtnClick(e){
        ...
        if(startDate && endDate || !startDate && !endDate) {
            ...
            if(click_gap != undefined) click_gap_set();
        } else if(startDate && !endDate) {
            let thisValue = tgDate.valueOf(),
                startValue = startDate.valueOf();
            if(select_gap != null && Math.abs(thisValue - startValue) > (60 * 60 * 24 * 1000 * (select_gap - 1))) {
                nToast('기간은 최대 '+ select_gap+'일 까지만 선택 가능합니다.');
                return;
            }
    ...
    // select_gap / click_gap 설정 변경시
    function option_update(new_option){
        select_gap      = new_option.select_gap != undefined ? new_option.select_gap : null;
        click_gap       = new_option.click_gap != undefined ? new_option.click_gap : null;
    }
    ...
    this.enabled_update = function(arr){
        enabled_arr = arr;
    }
    ...
    /** 옵션변경 */
    this.option_update = function(val){
        option_update(val);
    }
...
/** 달력 기능 - 월간 ================================================================== */
function nCalendarRangeScroll_mon(option){
    let inPage          = option.inPage ? option.inPage : false,
        inTarget        = typeof option.inTarget === 'string' ? document.querySelector(option.inTarget) : option.inTarget,
        inp_single      = option.inp_single ? option.inp_single : false,
        inp_single_cls  = option.inp_single_cls != undefined ? option.inp_single_cls : '.cal',
        calTitle        = option.calTitle,
        drawLimit       = option.drawLimit ? option.drawLimit : '2000',
        beforeLimit     = option.beforeLimit ? option.beforeLimit : null,
        select_gap      = option.select_gap ? option.select_gap : null,
        enabled_arr     = option.enabled_arr,
        activeConfirm   = option.activeConfirm;
    ...
    let now         = new Date(),
        today       = new Date(now.getFullYear(), now.getMonth(), now.getDate()),
        yearArr     = new Array(), // 오늘포함월 ~ 선택월 / 그 사이 날짜를 포함하는 연도 배열 */
        cal_height,
        dateBtns,
        limitDay,
        drawLimitYear = Number(drawLimit.split('-')[0]);
    ...
    // 그리기 설정 (기본달력 / inPage 달력) =====================================================================================
    // 달력 modal 구조 생성
	let newCal = createEle('div', ['cal-wrap', 'month']),
        inbox  = createEle('div', 'cal-cnt'),
        calTop = createEle('div', 'cal-top'), // 달력 상단 제어영역
        calTit = createEle('h2', 'cal-title'),
        btn_close = createEle('button', 'btn-cal-close');
    ...
    calInfo.appendChild(dateInfo);
    info_start.textContent = '시작월';
    info_end.textContent = '종료월';
    info_start.appendChild(startTx);
    info_end.appendChild(endTx);
    dateInfo.appendChild(info_start);
    dateInfo.appendChild(info_end);
    ...
    function calConfirm(){
        let start_tx = startTx.textContent,
            end_tx = endTx.textContent;
        if(inp_single == true) {
            inp_start.value = start_tx + ' ~ ' + end_tx;
            inp_start.dispatchEvent(changeEvt);
        } else {
            inp_start.value = start_tx;
            inp_end.value = end_tx;
            inp_start.dispatchEvent(changeEvt);
            inp_end.dispatchEvent(changeEvt);
        }
        if(typeof activeConfirm === 'function') activeConfirm();
    ...
    function inpGetDate(){
        ...
        if(inp_start.value.length > 0) startTx.textContent = inp_start.value;
        if(inp_end.value.length > 0) endTx.textContent = inp_end.value;
    }

    /** 입력된 날짜 여부 확인 - inPgae 일 경우 */
    function getDataDate(){
        let s_tx = startTx.textContent,
            e_tx = endTx.textContent;
        if(s_tx.length > 0) {
            startDate = convertToDate(s_tx + '-1', '-');
            startTx.textContent = s_tx;
        }
        if(e_tx.length > 0) {
            endDate = convertToDate(e_tx + '-1', '-');
            endTx.textContent = e_tx;
        }
    }

    let viewGap; // 이번달 - 시작일포함 달 간의 연도차이 숫자값
    /** viewGap 산출 - 이번달 - 시작일포함 달 간의 연도차이 숫자값 */
    function calcGap_mon(){
        if(startDate == undefined) { viewGap = 2; }
        else {
            let viewDateGap = (today.getFullYear() - startDate.getFullYear());
            viewGap = viewDateGap < 2 ? 2 : viewDateGap; // 계산값이 2 이하일 경우 2 고정
        }
    }

    /** 이번달 - 시작일 포함달 및 사이에 포함된 연도의 배열 생성 */
    function yearCalc(){
        let tempArr = new Array();
        for(let i=0; i<viewGap; i++){
            let gapDate = today.getFullYear() - i;
            tempArr.push(gapDate);
        }
        yearArr = tempArr.filter((item, idx) => tempArr.indexOf(item) === idx);
    }
    ...
    function calcPrevYear(){
        let nowFirst = yearArr[yearArr.length - 1],
            tempY   = nowFirst,
            tgDate  = new Date(tempY - 1, 0, 1);
        return tgDate.getFullYear();
    }
     ...
     */
    function calBtnOn(area){
        let btns = area.querySelectorAll('li button');
        Array.prototype.forEach.call(btns, function(btn){
            dateStateSet(btn);
            btn.addEventListener('click', dateBtnClick);
        });
        dateBtns = calArea.querySelectorAll('li button'); // 전체버튼 변수 업데이트
    }
    ...
    function preMonAdd(ent){
        ent.forEach((entry) => {
            if(entry.isIntersecting) {
                let tgDate = calcPrevYear();
                if(tgDate <= drawLimitYear) {
                    obsr1.disconnect();
                    return;
                }
                yearArr.push(tgDate);
                makeCalendar_mon(tgDate);
                setTimeout(function(){ // ios 보정용 settimeout
                    scrollArea.scrollTo(0, (cal_height + 25)); // 25 는 cal 에 설정된 margin-top 값
                }, 10)
            }
    ...
    function makeCalendar_mon(year) {        
        let div_cal   = createEle('div', 'cal');

        div_cal.insertAdjacentHTML('beforeend', '<p class="cal-year-mon">'+ year +'</p>');
        let ul_item = '<ul class="cal-month-list">';
        for(let m=1; m<13; m++){
            ul_item += '<li><button type="button" data-date="'+ year +'-'+ m +'" title="'+ year +'년 '+ m +'월">'+ m +'월</button></li>';
        }
        ul_item += '</ul>';
        div_cal.insertAdjacentHTML('beforeend', ul_item);
    ...
    function dateStateSet(e){
		let tg          = e,
			tgDate      = tg.getAttribute('data-date'),
            tgYear      = tgDate.split('-')[0],
            tgMon       = Number(tgDate.split('-')[1]),
            thisValue   = convertToDate(tgDate + '-1', '-').valueOf();
        if(thisValue > today.valueOf()) btn_dis(tg);
        if(beforeLimit != null && thisValue < limitDay.valueOf()) btn_dis(tg);
        if(tgYear == today.getFullYear() && tgMon == today.getMonth() + 1) tg.parentNode.classList.add('today');
    ...
    /** 선택 날짜 관련 클래스 전체 제거 */
    function selClassClear(){
        if(calArea.querySelector('li.start')) calArea.querySelector('li.start').classList.remove('start');
        if(calArea.querySelector('li.end')) calArea.querySelector('li.end').classList.remove('end');
        if(calArea.querySelector('li.select')) calArea.querySelector('li.select').classList.remove('select');
        let ranges = calArea.querySelectorAll('li.in-range');
        Array.prototype.forEach.call(ranges, function(range){ range.classList.remove('in-range') });
    }
    ...
    function onSetStartDate(date){
        startDate = date;
        let tx = startDate.getFullYear() + '-' + setZero(startDate.getMonth() + 1);
        startTx.setAttribute('data-date', tx);
        startTx.textContent = tx;
    }
    /** 종료일 설정 */
    function onSetEndDate(date){
        endDate = date;
        let tx = endDate.getFullYear() + '-' + setZero(endDate.getMonth() + 1);
        endTx.setAttribute('data-date', tx);
        endTx.textContent = tx;
    }

    /** 날짜 클릭 시 실행 */
    function dateBtnClick(e){
        let tg      = e.target,
            tgDate  = convertToDate(tg.getAttribute('data-date') + '-1', '-');
        if(startDate && endDate || !startDate && !endDate) {
            selClassClear();
            tg.parentNode.classList.add('start');
            endDate = null;
            ...
            let thisValue = tgDate.valueOf(),
                startValue = startDate.valueOf();
            if(select_gap != null && (thisValue - startValue) > (60 * 60 * 24 * 1000 * (select_gap - 1))) {
                nToast('기간은 최대 '+ select_gap+'일 까지만 선택 가능합니다.');
                return;
            }
            if( thisValue >= startValue ) {
                ...
                onSetEndDate(startDate);
                onSetStartDate(tgDate);
                let firstTd = calArea.querySelector('li.start');
                selClassClear();
                firstTd.classList.remove('start');
                firstTd.classList.add('end');
                tg.parentNode.classList.add('start');
    ...
    function dateRangeBgSet(){
        ...
        Array.prototype.forEach.call(dateBtns, function(dateBtn){
            let dateBtnVal = convertToDate(dateBtn.getAttribute('data-date') + '-1', '-').valueOf();
            dateBtnVal >= startValue && dateBtnVal <= endValue ? dateBtn.parentNode.classList.add('in-range') : dateBtn.parentNode.classList.remove('in-range');
        });
    };

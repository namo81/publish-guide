...
function nlayer(option){
	...
	let body = document.querySelector('body'),
		...
		tg_focus_dom = tg_layer.querySelector('a, button, input, select, textarea'); // 레이어 내 처음 포커스될 요소

	if(option.btn) layer.btn_open = typeof option.btn === 'string' ? document.querySelectorAll(option.btn) : option.btn;

	// 레이어 오픈 후 첫 focus 대상을 타이틀로 할 경우
	...
	function pageSet(){
		...
		tabEle.forEach(function(ele){
			...
			//if(ele.getAttribute('inert')) ele.classList.add('inert');
	...
	function layerHideClose(){
		layerHide();
		if(typeof option.activeClose === 'function') option.activeClose(tg_layer, layer.btn_active);
	}
	/** 레이어 닫기 (confirm) */
	function layerHideConfirm(){
		if(typeof option.activeConfirm === 'function') {
			if(option.activeConfirm(tg_layer, layer.btn_active) != false) layerHide();
		} else layerHide();
	}
	...
	/** 레이어 보기 */
	function layerShow(){
		if(typeof option.activeShowBefore === 'function') option.activeShowBefore(tg_layer, layer.btn_active);
		setTimeout(function(){ // alert / confirm 일 경우 show 클래스 관련 transition 적용을 위한 delay
			...
			pageSet();
			// tg_focus_dom.focus(); --> focus 로 인해 화면 밖 > 안으로 이동하는 모션 무시됨 - 필요 시 transitionend 이벤트 추가 후 적용
			if(layer.btn_active != null) layer.btn_active.setAttribute('aria-expended', true);
			if(typeof option.activeShow === 'function') option.activeShow(tg_layer, layer.btn_active);
		}, 10)
	}

...
$.fn.nCalendarWeek = function(option){
	...
	var $inp = set.inp, // 초기 모든 inp 에 버튼 추가를 위한 변수
		...
		$showBoolean = true; // show 방식에 따른 변수 - ie 에서 focus/both 일 경우 선택된 다음 자동으로 기존 input에 focus 될때 달력이 다시 생성됨.

	//body 에 달력 div 생성
	if(!$('#cal-week-wrap').length) {
		$('body').append('<div id="cal-week-wrap" class="cal-wrap"><div id="cal-area"></div></div>');
		$wrap = $('#cal-week-wrap'),
		...
		if(set.changeMon == true){
			...
			$monthSelect = $wrap.find('.sel-month'),
			$prevM = $wrap.find('.cal-btn.prev'),
			$nextM = $wrap.find('.cal-btn.next');
			if(set.changeYear == true) $yearSelect = $wrap.find('.sel-year');
		}
	...
	var calendarOn = function(){ // 설정된 연,월,일로 달력 생성 및 기능 적용 (show X)
		...
		chkYoil();
		makeCalendar(yoil, nalsu[month],year,month + 1);
	...
	function inpWrite(){ 
		...
		$btnDay.each(function(){
			$(this).click(function(){
				var $activeDate = new Date($(this).parent().attr('data-year'), Number($(this).parent().attr('data-month')), Number($(this).text())),
				...
				if(set.inpWeekNum) $tgInp.val(''+$startYear+'년 '+weekNo+'주');
				else $tgInp.val(''+$startYear+'-'+$startMonth+'-'+$startDay+' ~ '+$endYear+'-'+$endMonth+'-'+$endDay+'');
				calhide();
			});
		});
	}
	...
	function setToActiveDay(){
		...
		if($tgInp.val().length > 0){
			$btnDay.each(function(){
				var $activeMonth = $(this).parent().attr('data-month'),
					$activeYear = $(this).parent().attr('data-year');
		...
		for(var r=0; r < ju; r++){			
			...
			for(var col=0; col < 7; col++){
				if(r == 0 && currentCell < yoil){					
					var noprev = nalsu[prevMonth] - ((yoil -1)-currentCell);
					str += "<td data-month="+ prevMonth +" data-year="+ prevYear +" class='prevMonth'><button type='button'>" + noprev + "</button></td>";
					currentCell++;
				} else if( r == (ju -1) && no > maxday){
					str += "<td data-month="+ nextMonth +" data-year="+ nextYear +" class='nextMonth'><button type='button'>" + nextNo + "</button></td>";
					nextNo++;
				} else {
					str += "<td data-month="+ (month -1) +" data-year="+ year +"><button type='button'>" + no + "</button></td>";
					no++;
				}
			}
			str += "</tr>";
		...
		if(set.showWeekNum) {  // 주차 수 표기일 경우 추가
			$cal.find('tbody tr').each(function(){
				var thu =  $(this).children('td').eq(5),
					...
					thuYear = thu.attr('data-year'),
					thuMon = thu.attr('data-month'),

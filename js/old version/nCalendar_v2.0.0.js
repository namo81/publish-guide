					...
					for(o=0; o< monOpts.length; o++){
						...
						if(opts.todayLimit == true){
							if(opts.limitType == 'after') {
								year == thisYear && monOpts[o].value > thisMonth + 1 ? monOpts[o].style.display = 'none' : monOpts[o].style.display = 'block';
							} else {
								year == thisYear && monOpts[o].value < thisMonth + 1 ? monOpts[o].style.display = 'none' : monOpts[o].style.display = 'block';							
							}
						}
		...
		var calSetMonth = function(){
			...
			function limitSet(e){ // 오늘 날짜 제한 기능 관련
				var tg = e,
					...
					tgMonth = tg.getAttribute('data-month');
				if(opts.limitType == 'after') {
					if(tgYear > thisYear || tgYear == thisYear && tgMonth > thisMonth + 1) tg.disabled = true;
				} else {
					if(tgYear < thisYear || tgYear == thisYear && tgMonth < thisMonth + 1) tg.disabled = true;
				}
			}
			...
			function limitPNSet(){
				if(opts.limitType == 'after') {
					year >= thisYear ? nextM.disabled = true : nextM.disabled = false;
				} else {
					year <= thisYear ? prevM.disabled = true : prevM.disabled = false;							
				}
			}
